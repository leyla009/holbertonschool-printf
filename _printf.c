...
/**
 * _printf - Custom printf function that handles flags, width,
 * precision, length modifiers, and custom specifiers.
	...
	for (i = 0; format[i]; i++)
	{
		if (format[i] == '%')
		{
			       ...
			       format[i + 1] == '-')
			{
				...
				else if (format[i] == '0') flags |= 8;
				else if (format[i] == '-') flags |= 16;
			}
			/* 2. Parse Width */
			if (format[i + 1] == '*')
			{
